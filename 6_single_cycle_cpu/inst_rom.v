...
//*************************************************************************
//   > 文件名: inst_rom.v
//   > 描述  ：异步指令存储器模块，采用寄存器搭建而成
//   >         内嵌好指令，只读，异步读（已扩容并注入计算Fib(10)的程序）
//   > 作者  : LOONGSON (Modified for lab requirements)
//   > 日期  : 2016-04-14
//*************************************************************************
module inst_rom(
    input      [4 :0] addr,      // 指令地址（字地址）
    output reg [31:0] inst       // 指令内容
    );
    
    // 原始总共 20 条指令，现扩容至 29 条指令（索引 0 ~ 28）
    wire [31:0] inst_rom[28:0];  
    ...
    //------------------ 后续新增：计算 Fib(10) 的 MIPS 汇编机器码 ------------------
    assign inst_rom[19] = 32'h240D0009; // 4CH: addiu $13, $0, 9   | 初始化循环计数器 n = 9
    assign inst_rom[20] = 32'h240E0000; // 50H: addiu $14, $0, 0   | 初始化 Fib(0) = 0
    assign inst_rom[21] = 32'h240F0001; // 54H: addiu $15, $0, 1   | 初始化 Fib(1) = 1
    assign inst_rom[22] = 32'h24110001; // 58H: addiu $17, $0, 1   | $17 = 1 (供后续 sub 计数自减使用)
    
    // LOOP 循环体
    assign inst_rom[23] = 32'h01CF8021; // 5CH: addu  $16, $14, $15| [LOOP] next = f0 + f1
    assign inst_rom[24] = 32'h01E07021; // 60H: addu  $14, $15, $0 | f0 = f1
    assign inst_rom[25] = 32'h02007821; // 64H: addu  $15, $16, $0 | f1 = next
    assign inst_rom[26] = 32'h01B16822; // 68H: sub   $13, $13, $17| 验证新指令：n = n - 1
    assign inst_rom[27] = 32'h1DA0FFFB; // 6CH: bgtz  $13, -5       | 验证新指令：若 n > 0 则跳回 5CH (LOOP)
    
    // 大循环复位
    assign inst_rom[28] = 32'h08000000; // 70H: j     00H         | 原 4CH 处的复位跳转，移至程序末尾
